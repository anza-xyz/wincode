    ...
    T: Copy + 'static,
{
    ...
    fn write(writer: &mut impl Writer, src: &Self::Src) -> WriteResult<()> {
        ...
        unsafe { Ok(writer.write_from_t(src)?) }
    }
}
    ...
    T: Copy + 'static,
{
    ...
    fn read(reader: &mut impl Reader<'de>, dst: &mut MaybeUninit<Self::Dst>) -> ReadResult<()> {
        // SAFETY: `T` is plain ol' data.
        unsafe { Ok(reader.read_into_t(dst)?) }
    }
}
    ...
    T: Copy + 'static,
{
    ...
    fn write(writer: &mut impl Writer, src: &Self::Src) -> WriteResult<()> {
        ...
        unsafe { Ok(writer.write_from_slice_t(src.as_slice())?) }
    }
}
    ...
    T: Copy + 'static,
{
    ...
    fn read(reader: &mut impl Reader<'de>, dst: &mut MaybeUninit<Self::Dst>) -> ReadResult<()> {
        ...
        unsafe { reader.read_into_slice_t(spare_capacity)? };
    ...
    ($container:ident => $target:ident) => {
            ...
            T: Copy + 'static,
        {
            ...
            fn write(writer: &mut impl Writer, src: &Self::Src) -> WriteResult<()> {
                ...
                unsafe { Ok(writer.write_from_slice_t(&src[..])?) }
            }
        }
            ...
            T: Copy + 'static,
        {
            ...
            fn read(
                ...
                unsafe {
                    reader.read_into_slice_t(&mut *ptr)?;
                }
    ...
    T: Copy + 'static,
{
    ...
    fn write(writer: &mut impl Writer, src: &Self::Src) -> WriteResult<()> {
        ...
        unsafe {
            ...
            writer.write_from_slice_t(front)?;
            writer.write_from_slice_t(back)?;
        }
        Ok(())
    }
}
    ...
    Len: SeqLen,
{
    ...
    fn write(writer: &mut impl Writer, src: &Self::Src) -> WriteResult<()> {
        Len::write(writer, src.len())?;
        // SAFETY: Caller ensures `T` is plain ol' data.
        unsafe { writer.write_from_slice_t(src.as_slice())? }
        Ok(())
    }
}
