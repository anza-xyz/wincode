...
pub trait Serialize: SchemaWrite {
    ...
    fn serialize(src: &Self::Src) -> WriteResult<Vec<u8>> {
        ...
        Self::serialize_into(src, &mut writer)?;
        let len = writer.len();
        unsafe {
    ...
    #[inline]
    fn serialize_into(src: &Self::Src, dst: &mut impl Writer) -> WriteResult<()> {
        Self::write(dst, src)?;
        dst.finish()?;
        Ok(())
    }
...
#[inline]
pub fn serialize_into<T>(src: &T, dst: &mut impl Writer) -> WriteResult<()>
where
    T: SchemaWrite<Src = T> + ?Sized,
{
