use {
    ...
    syn::{parse::Parser as _, parse_quote, DeriveInput, GenericParam, LitInt, Path, Type},
};

pub(crate) fn impl_struct_extensions(args: &SchemaArgs, crate_name: &Path) -> Result<TokenStream> {
...
/// Parses `#[wincode::struct_extensions(internal)]` and returns `true` if `internal` is present.
///
/// Used tests in `wincode` to correctly resolve the crate path.
fn parse_is_internal(attr: TokenStream) -> Result<bool> {
    ...
    let parser = syn::meta::parser(|meta| {
        if meta.path.is_ident("internal") {
            ...
            Err(meta.error("unsupported argument for `struct_extensions`; expected `internal`"))
        }
    });
...
pub(crate) fn generate(attr: TokenStream, input: DeriveInput) -> Result<TokenStream> {
    let mut args = SchemaArgs::from_derive_input(&input)?;
    if parse_is_internal(attr)? {
        args.internal = true;
    }

    let crate_name = get_crate_name(&args);
    let tokens = impl_struct_extensions(&args, &crate_name)?;
    Ok(quote! {
        #input
        #tokens
    })
